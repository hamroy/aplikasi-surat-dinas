      ...
      <div class="card-body">
        ...
        <table class="table table-striped table-bordered">
          ...
          ?>
            <tr>
              <td>
                <font size=""><?php echo $tampil['idj'] ?></font>
              </td>
              <td>
                <font size=""><?php echo $tampil['nmj'] ?></font>
              </td>
              <td>
                <button class="btn btn-info btn-sm open_modal" id='<?php echo  $tampil['idj']; ?>'><i class="fas fa-edit"> Edit</i></button>
                <?php
                $akses = $this->session->userdata('lvl_akses');
                if ($akses == 1) {
                ?>
                  <button class="btn btn-danger btn-sm open_modal" onclick="confirm_modal('<?= base_url() ?>C_master/jabatanDel?&modal_id=<?= $tampil['idj'] ?>');"><i class="far fa-times-circle"> Hapus</i></button>
                <?php
                }
                ?>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="<?= base_url() ?>C_master/jabatan_save" name="modal_popup" enctype="multipart/form-data" method="POST">
...
<script type="text/javascript">
  $(document).ready(function() {
    $(".open_modal").click(function(e) {
      var m = $(this).attr("id");
      $.ajax({
        url: "<?= base_url() ?>C_master/jabatanEdit",
        type: "GET",
        data: {
          modal_id: m,
        },
